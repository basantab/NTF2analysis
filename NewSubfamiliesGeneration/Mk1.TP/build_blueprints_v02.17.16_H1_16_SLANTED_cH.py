...
topol = "L[1-1]H[13-13]L[2-2]H[7-7]L[5-5]E[4-4]L[2-2]E[4-4]L[1-1]H[14-14]L[2-2]E[10-10]L[2-2]E[10-10]L[2-2]E[12-12]L[2-2]E[9-9]L[2-2]H[11-11]L[1-1]"
...
for comb in combinations:
	...
	for j,bulcomb in enumerate(bcombinations):
			...
			fileout_min = open('cst1_min','w') # E2//E3
                        ...
                        global_bend=60.0,global_bend_tol=5.0,\
			global_twist=40.0,global_twist_tol=5.0,\
			constraint_type='bounded' )

                        st2 = RegularStrandCurvature(level=2,blueprint=blue,strand='E2',\
			global_twist=40.0,global_twist_tol=5.0,\
                        global_bend=60.0,global_bend_tol=5.0,constraint_type='bounded' )
			'''
			st1 = RegularStrandCurvature(level=2,blueprint=blue,strand='E1',\
			...
			st = PairConstraints(H4c,H3c-7,5,2,"H4E4") ; fileout.write(st);
			st = PairConstraints(H4c,H3c-4,5,2,"H4E4b") ; fileout.write(st);
			#st = PairConstraints(H4c-1,E4c+3,6,2,"H4E4") ; fileout.write(st);
			print "Strand 5: %d"%(len(seg3.bp_data))
			st = AngleConstraints(E6c-2,L10,H4c,45,5,"E6H4") ; fileout.write(st); fileoutb.write(st);
			...
			st = PairConstraints(h1n,e3c,8,3,"H1E3n") ; fileout.write(st); fileoutb.write(st); fileoutc.write(st);
			...
			#Additional for TPs:
			st = PairConstraints(h1c,bulge1-1,9,2,"H1nE3") ; fileout.write(st); fileoutb.write(st); fileoutc.write(st);
			st = PairConstraints(h2n,loop6,12,2,"H2nloop6") ; fileout.write(st); fileoutb.write(st);
			st = PairConstraints(h2n+1,loop6-6,4,2,"H2nH3") ; fileout.write(st); fileoutb.write(st);
			st = AngleConstraints(h1c-6,h1c+1,h2c,37,13,"E6H4b") ; fileoutc.write(st);
